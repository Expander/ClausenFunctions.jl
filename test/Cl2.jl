...
@testset "cl2 BigFloat" begin
    ...
    setprecision(BigFloat, 512) do
        # Cl₂(π/6)
        @test ClausenFunctions.cl2(BigFloat(π)/6) ≈ big"0.86437913105389274962503631519021947866818857640368970418203768977532471558206" rtol=big"1e-60"

        # Cl₂(π/4)
        @test ClausenFunctions.cl2(BigFloat(π)/4) ≈ big"0.98187215105020335671792454306019566713079097166073046157661313465315566504977" rtol=big"1e-60"

        # Cl₂(π/3)
        @test ClausenFunctions.cl2(BigFloat(π)/3) ≈ big"1.01494160640965362502120255427452028594168930753029979201748910677659747625824" rtol=big"1e-60"

        # Cl₂(π/2) = Catalan constant G
        @test ClausenFunctions.cl2(BigFloat(π)/2) ≈ big"0.91596559417721901505460351493238411077414937428167213426649811962176301977625" rtol=big"1e-60"
        ...
        # Cl₂(2π/3)
        @test ClausenFunctions.cl2(2*BigFloat(π)/3) ≈ big"0.67662773760643575001413503618301352396112620502019986134499273785106498417216" rtol=big"1e-60"

        # Special values
        @test ClausenFunctions.cl2(big"0.0") == big"0.0"
        @test ClausenFunctions.cl2(BigFloat(π)) ≈ big"0.0" atol=big"1e-60"

        # Antisymmetry: Cl₂(-x) = -Cl₂(x)
        @test ClausenFunctions.cl2(-BigFloat(π)/4) ≈ -ClausenFunctions.cl2(BigFloat(π)/4) rtol=big"1e-60"

        # Periodicity: Cl₂(x + 2π) = Cl₂(x)
        @test ClausenFunctions.cl2(big"1" + 2*BigFloat(π)) ≈ ClausenFunctions.cl2(big"1") rtol=big"1e-40"
    end
